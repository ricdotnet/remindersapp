<template>
	<div class="mb-4 mt-4 cursor-pointer" v-on:click="liveCurrentTheme = !liveCurrentTheme; onClick()">
		<div id="sun" class="text-gray-100 transition duration-300" :class="{ 'hidden' : liveCurrentTheme }">
		...
		<div id="moon" class="text-gray-600 transition duration-300" :class="{ 'hidden' : !liveCurrentTheme }">
			<svg class="w-7 h-7 fill-current" viewBox="0 0 20 20">
...
export default {
	...
	beforeMount() {
		this.liveCurrentTheme = this.currentTheme
	},
	mounted() {
		if(this.liveCurrentTheme === 'false') {
	...
	methods: {
		onClick() {
			console.log(this.liveCurrentTheme)
			if(this.liveCurrentTheme === true) {
				document.body.style.background = '#E0E7FF';
				console.log('setting light')
			} else

			if(this.liveCurrentTheme === false) {
				document.body.style.background = '#312E81';
				console.log('setting dark')
			}

			localStorage.setItem('light', this.liveCurrentTheme);
			this.$store.currentTheme = this.liveCurrentTheme;
		}
	}
}
</script>